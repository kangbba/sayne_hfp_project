...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        ...
        case ESP_GATTS_REG_EVT:
            ...
            memcpy(service_id.id.uuid.uuid.uuid128, adv_service_uuid128, ESP_UUID_LEN_128);
        ...
        case ESP_GATTS_CREATE_EVT:
            ...
            memcpy(tx_char_uuid.uuid.uuid128, (uint8_t[]){0xbe, 0xb5, 0x48, 0x3e, 0x36, 0xe1, 0x46, 0x88, 0xb7, 0xf5, 0xea, 0x07, 0x36, 0x1b, 0x26, 0xa9}, ESP_UUID_LEN_128);

            esp_bt_uuid_t rx_char_uuid = {
                .len = ESP_UUID_LEN_128,
            };
            memcpy(rx_char_uuid.uuid.uuid128, (uint8_t[]){0xbe, 0xb5, 0x48, 0x3e, 0x36, 0xe1, 0x46, 0x88, 0xb7, 0xf5, 0xea, 0x07, 0x36, 0x1b, 0x26, 0xa8}, ESP_UUID_LEN_128);
            ...
            ESP_LOGI(GATTS_TAG, "Adding RX characteristic, result: %d", add_char_ret);
            if (add_char_ret) {
            ...
            add_char_ret = esp_ble_gatts_add_char(param->create.service_handle, &tx_char_uuid,
                                                  ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                                                  char_property, NULL, NULL);
            ESP_LOGI(GATTS_TAG, "Adding TX characteristic, result: %d", add_char_ret);
            if (add_char_ret) {
